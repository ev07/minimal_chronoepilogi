"""
Modification of the mass_ts pypi package source code. 
The original code is under Apache 2.0 license, which we include here.
This license is applicable only to the present file.

As per the license terms (4.c), we include all necessary references to the original package:
 - Abdullah Mueen, Yan Zhu, Michael Yeh, Kaveh Kamgar, Krishnamurthy Viswanathan,
   Chetan Kumar Gupta and Eamonn Keogh (2015), The Fastest Similarity Search Algorithm
   for Time Series Subsequences under Euclidean Distance, 
   URL: http://www.cs.unm.edu/~mueen/FastestSimilaritySearch.html
 - https://github.com/tylerwmarrs/mass-ts
 - Development Lead: Tyler Marrs <tylerwmarrs@gmail.com>
...
def normalize(x):
    #x should have shape (time,) or shape (time,variables)
    a, b = (x-np.mean(x,axis=0)), np.std(x,axis=0)
    return np.divide(a,b,where=(b!=0),out=np.zeros_like(a))  # 'where' to handle nans from std=0
...
def mass2_modified(ts, query):
    ...
    padding = np.ones_like(x)
    padding[:,-meanx.shape[-1]:] = meanx
    meanx = padding
    ...
    padding = np.zeros_like(x)
    padding[:,-sigmax.shape[-1]:] = sigmax
    sigmax = padding

    y = np.append(np.flip(y), np.zeros([1, n - m]))
    
    # fft computation is currently the major bottleneck of the function
    z = np.fft.fft(x,axis=-1) * np.fft.fft(y,axis=-1)
